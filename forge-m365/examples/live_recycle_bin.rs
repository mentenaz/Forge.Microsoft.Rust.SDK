...
async fn main() {
    ...
    // Read-only: this example deliberately does not restore/delete anything,
    // since recycle-bin deletion is not reversible.
    match forge_m365_sp_recycle_bin::get_recycle_bin_items(&client, &site_url).await {
        Ok(items) => println!("{} recycle bin item(s):\n{items:#?}", items.len()),
        Err(e) => {
            eprintln!("get_recycle_bin_items FAILED: {e}");
            std::process::exit(1);
        }
    }
}
