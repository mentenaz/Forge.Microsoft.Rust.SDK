...
async fn main() {
    ...
    let groups = match forge_m365_sp_site_groups::get_site_groups(&client, &site_url).await {
        Ok(groups) => {
            println!("{} group(s):\n{groups:#?}", groups.len());
            groups
        }
        Err(e) => {
            eprintln!("get_site_groups FAILED: {e}");
            std::process::exit(1);
        }
    };

    if let Some(first) = groups.first() {
        match forge_m365_sp_site_groups::get_group_members(&client, &site_url, first.id).await {
            Ok(members) => println!(
                "{} member(s) of '{}':\n{members:#?}",
                members.len(),
                first.title
            ...
            Err(e) => {
                eprintln!("get_group_members FAILED: {e}");
                std::process::exit(1);
            }
        }
    }
