...
async fn main() {
    ...
    match forge_m365_sp_content_types::get_web_content_types(&client, &site_url).await {
        Ok(cts) => println!("{} web content type(s):\n{cts:#?}", cts.len()),
        Err(e) => {
            eprintln!("get_web_content_types FAILED: {e}");
            std::process::exit(1);
        }
    }

    if let Ok(list_title) = std::env::var("M365_LIST_TITLE") {
        match forge_m365_sp_content_types::get_list_content_types(&client, &site_url, &list_title)
            .await
        {
            Ok(cts) => println!("{} list content type(s):\n{cts:#?}", cts.len()),
            Err(e) => {
                eprintln!("get_list_content_types FAILED: {e}");
                std::process::exit(1);
            }
        }
    } else {
        eprintln!("\nset M365_LIST_TITLE to also list that list's content types");
    }
}
