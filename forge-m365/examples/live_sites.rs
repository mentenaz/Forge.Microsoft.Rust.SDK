...
async fn main() {
    ...
    match forge_m365_sp_sites::get_site(&client, &site_url).await {
        Ok(site) => {
            println!("site: {site:#?}");
        }
        Err(e) => {
            eprintln!("get_site FAILED: {e}");
            std::process::exit(1);
        }
    }

    match forge_m365_sp_sites::get_root_web_url(&client, &site_url).await {
        Ok(url) => println!("root web url: {url}"),
        Err(e) => {
            eprintln!("get_root_web_url FAILED: {e}");
            std::process::exit(1);
        }
    }
}
