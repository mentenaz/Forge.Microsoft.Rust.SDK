...
async fn main() {
    ...
    match forge_m365_sp_comments::get_comments(&client, &site_url, &list_title, item_id).await {
        Ok(comments) => println!("{} comment(s):\n{comments:#?}", comments.len()),
        Err(e) => {
            eprintln!("get_comments FAILED: {e}");
            std::process::exit(1);
        }
    }

    // Write round trip is opt-in: it mutates real data, so this example only
    // ever touches a comment it creates itself.
    if std::env::var("M365_COMMENTS_WRITE_TEST").ok().as_deref() != Some("1") {
        eprintln!(
            "\nset M365_COMMENTS_WRITE_TEST=1 to also round-trip add/delete on a throwaway comment"
        );
        return;
    }

    let added = forge_m365_sp_comments::add_comment(
        &client,
        &site_url,
        &list_title,
        item_id,
        "forge-m365 live_comments test comment",
    )
    .await
    .unwrap_or_else(|e| {
        eprintln!("add_comment FAILED: {e}");
        std::process::exit(1);
    });
    println!("added: {added:#?}");

    forge_m365_sp_comments::delete_comment(&client, &site_url, &list_title, item_id, &added.id)
        .await
        .unwrap_or_else(|e| {
            eprintln!("delete_comment FAILED: {e}");
            std::process::exit(1);
        });
    println!("deleted comment {}", added.id);
}
