...
async fn main() {
    ...
    let query_text = std::env::var("M365_SEARCH_QUERY").unwrap_or_else(|_| "*".to_string());
    ...
    match forge_m365_sp_search::search(
        &client,
        &site_url,
        &query_text,
        Some(10),
    ...
    .await
    {
        Ok(results) => {
            println!(
                "{} row(s) of {} total ({}ms)",
                results.row_count, results.total_rows, results.elapsed_time
            );
