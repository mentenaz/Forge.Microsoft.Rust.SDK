...
async fn main() {
    ...
    let views = match forge_m365_sp_views::get_views(&client, &site_url, &list_title).await {
        Ok(views) => {
            println!("{} view(s):\n{views:#?}", views.len());
            views
        }
        Err(e) => {
            eprintln!("get_views FAILED: {e}");
            std::process::exit(1);
        }
    };

    if let Some(default_view) = views.iter().find(|v| v.default_view) {
        match forge_m365_sp_views::get_view_fields(
            &client,
            &site_url,
            &list_title,
            &default_view.id,
        ...
        .await
        {
            Ok(fields) => println!("default view fields: {:?}", fields.items),
            Err(e) => {
                eprintln!("get_view_fields FAILED: {e}");
                std::process::exit(1);
            }
        }
    }
