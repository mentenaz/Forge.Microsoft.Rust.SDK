...
async fn main() {
    ...
    // App-only tokens have no signed-in user, so this is expected to fail
    // (or return the app identity, depending on tenant config) unless run
    // with a delegated token instead.
    match forge_m365_sp_site_users::get_current_user(&client, &site_url).await {
        Ok(user) => println!("current user: {user:#?}"),
        Err(e) => eprintln!("get_current_user FAILED (expected under app-only auth): {e}"),
    }

    match forge_m365_sp_site_users::get_site_users(&client, &site_url).await {
        Ok(users) => println!("{} user(s):\n{users:#?}", users.len()),
        Err(e) => {
            eprintln!("get_site_users FAILED: {e}");
            std::process::exit(1);
        }
    }
}
