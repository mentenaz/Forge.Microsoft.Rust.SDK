...
Workspace has 19 crates: `forge-m365` (facade), `-core` (Error, Surface,
...
`-sp-attachments`, `-sp-hubsites`, `-sp-recycle-bin`. Escalation-ladder
tests pass (`cargo test --workspace`, 69 tests as of `sp-recycle-bin`).
...
`sp-hubsites`, `sp-recycle-bin` ported (read-only + list/item/file/folder/
attachment CRUD + role assignments/definitions + recycle-bin
restore/delete; write ops beyond that — site/list/web create-delete,
...
setContent/recycle, recycle-bin bulk ops — are deferred and noted in each
crate's README). `sp-site-groups` reuses `SiteUserInfo` from
`sp-site-users` (first cross-crate type reuse). ~24 PnPjs `sp/*` packages
remain untouched (`fields` — largest remaining at ~600 lines — `sharing`,
`forms`, `navigation`, `comments`, `appcatalog`, `site-designs`,
`site-scripts`, ...).
